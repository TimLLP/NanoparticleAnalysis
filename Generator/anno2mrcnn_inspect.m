...
close all,clc
clear;
% % run only once to change the filder of all matlab annotated image
% annodir = '/home/hadmatter/Desktop/labeler_results/labels/Unet2';
% cd(annodir)
% img_path = dir(annodir);
%  
% img_path([1,2])=[];
% for i = 1:length(img_path)
%     img_fname = [img_path(i).name,'/','PixelLabelData'];
%     img_name = dir([img_fname '/' '*.png']); 
%     img = imread([img_fname '/' img_name.name]);
%     img(find(img==1))=uint8(255);
%     imwrite(img,[img_fname(4:6),'.png']);
%     copyfile('*.png','/home/hadmatter/Desktop/labeler_results/labels/manual/');
% end
