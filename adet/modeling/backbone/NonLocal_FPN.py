...
class NonLocalAttention(nn.Module):
    ...
    def forward(self, input):
        ...
        score = nn.matmul(x_embed_1, x_embed_2)
        score = F.softmax(score, dim=2)
        x_assembly = x_assembly.view(N, -1, H * W).permute(0, 2, 1)
        x_final = nn.matmul(score, x_assembly)
...
class NLocal_FPN(Backbone):
    ...
    def __init__(
        ...
        Args:
            ...
            fuse_type (str): types for fusing the top down features and the lateral
                ...
                which takes the element-wise mean of the two.
        """
        super(NLocal_FPN, self).__init__()
        assert isinstance(bottom_up, Backbone)
    ...
    def forward(self, x):
        ...
        for features, lateral_conv, output_conv in zip(
            ...
            prev_features = lateral_features + top_down_features
            # #every level output
            # fpn_out_features = output_conv(prev_features)
            # if fpn_out_features.shape == bottom_shape:
            #     middle = fpn_out_features
            # middle += F.interpolate(fpn_out_features,size=bottom_shape,mode="nearest")
            if self._fuse_type == "avg":
                 prev_features /= 2
            results.insert(0, output_conv(prev_features))
        ...
        if self.top_block is not None:
            ...
            results.extend(self.top_block(top_block_in_feature))
        middle = results[0]
        bottom_shape = middle.shape[2:]
        #merging the different scale features
        #first interpolate the same size in all level
        #second, added
        for i in range(0,len(results)):
               middle += F.interpolate(results[i],size = bottom_shape,mode="nearest")
        #Nonlocal Attention
        middle = self.nlocal(middle)
        #the feature intergrated to orignal size
        for i in len(results):
            results[i] = results[i] + self.downsample(middle)
        assert len(self._out_features) == len(results)
        return dict(zip(self._out_features, results))
...
def build_resnet_fpn_nonlocal_backbone(cfg, input_shape: ShapeSpec):
    """
    Args:
