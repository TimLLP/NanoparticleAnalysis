...
from .blenders import build_blender
from .basis_modules import build_basis_module
...
class Tiny_Conv(nn.Module):
    def __init__(self, channel):
        ...
        self.conv1 = BasicConv(in_planes=3, out_planes=channel, kernel_size=3, stride=1, padding=1)
        self.max_pool = nn.MaxPool2d(3, stride=2, padding=1, ceil_mode=False)
        self.conv2 = BasicConv(in_planes=channel, out_planes=channel, kernel_size=7, stride=1, padding=3)
        self.conv3 = BasicConv(in_planes=channel, out_planes=channel, kernel_size=3, stride=1, padding=1)
        self.conv4 = BasicConv(in_planes=channel, out_planes=channel, kernel_size=3, stride=1, padding=1)
        self.conv5 = BasicConv(in_planes=channel, out_planes=channel, kernel_size=3, stride=1, padding=1)
        self.conv6 = BasicConv(in_planes=channel, out_planes=channel, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        out5= x
        x = self.conv6(x)
        out = out1 + out2 + out3 + out4 + out5 + x
        return out

@META_ARCH_REGISTRY.register()
class ModifiedBlendMask(nn.Module):
    """
    Main class for ModifiedBlendMask architectures (see https://arxiv.org/abd/1901.02446).
    """
    ...
    def __init__(self, cfg):
        ...
        self.instance_loss_weight = cfg.MODEL.ModifiedBlendMasK.INSTANCE_LOSS_WEIGHT

        self.backbone = build_backbone(cfg)
        self.proposal_generator = build_proposal_generator(cfg, self.backbone.output_shape())
        self.blender = build_blender(cfg)
        self.BASIS_MODULES = build_basis_module(cfg, self.backbone.output_shape())
        self.tiny_features = Tiny_Conv(channel=256)
        ...
        attn_size = cfg.MODEL.ModifiedBlendMasK.ATTN_SIZE
    ...
    def forward(self, batched_inputs):
        ...
        tiny_feature = self.tiny_features(images.tensor)
        ...
        if "basis_sem" in batched_inputs[0]:
            ...
            basis_sem = None
        basis_out, basis_losses = self.BASIS_MODULES(tiny_feature, features, basis_sem)
        ...
        for i, (detector_result, input_per_image, image_size) in enumerate(zip(
            ...
            if self.BASIS_MODULES.visualize:
